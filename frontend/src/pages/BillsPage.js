...
export default function BillsPage() {
  ...
  const pageTitle = type === 'company' ? 'Contas Empresariais' : 'Contas Particulares';
  ...
  const [deleteDialog, setDeleteDialog] = useState({ open: false, billId: null });
  ...
  const handleStatusChange = async (billId, newStatus) => {
    try {
      ...
      toast.success(`Conta marcada como ${newStatus === 'paid' ? 'paga' : 'em aberto'}!`);
      fetchData();
    } catch (error) {
  ...
  const filteredBills = bills.filter(bill => {
    ...
    if (filters.city && !bill.city.toLowerCase().includes(filters.city.toLowerCase())) return false;
    if (filters.search && !bill.account.toLowerCase().includes(filters.search.toLowerCase())) return false;
    return true;
  });
  ...
  return (
    <div className="min-h-screen flex flex-col app-container">
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        <div className="fade-in">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 space-y-4 sm:space-y-0">
            <div>
              <h1 className="text-4xl font-bold text-gray-800 mb-2" data-testid="page-title">{pageTitle}</h1>
              <p className="text-gray-600">Gerencie suas contas {type === 'company' ? 'empresariais' : 'particulares'}</p>
            </div>
            {isAdmin && (
              <Button
                onClick={() => {
          ...
          <div className="bg-white rounded-xl shadow-md p-6 mb-6 border border-purple-100">
            ...
            {(filters.status || filters.category || filters.city || filters.search) && (
              <Button
                variant="ghost"
                size="sm"
            ...
            <div className="overflow-x-auto">
              <table className="w-full" data-testid="bills-table">
                <thead className="bg-gradient-to-r from-purple-50 to-blue-50">
                  <tr>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Conta</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredBills.length === 0 ? (
                    ...
                    filteredBills.map((bill) => (
                      <tr key={bill.id} className="hover:bg-purple-50 transition-colors" data-testid={`bill-row-${bill.id}`}>
                        <td className="px-6 py-4">
                          <div>
                            <p className="font-medium text-gray-800">{bill.name || bill.account}</p>
                            {bill.observations && (
                              <p className="text-sm text-gray-500">{bill.observations}</p>
                            )}
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <span className="font-semibold text-gray-800">R$ {bill.amount.toFixed(2)}</span>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(bill)}`}>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center justify-center space-x-2">
                            <Button
                              size="sm"
                              variant="ghost"
                              onClick={() => handleStatusChange(bill.id, bill.status === 'paid' ? 'open' : 'paid')}
                              className={`${
                                bill.status === 'paid'
                                  ? 'text-yellow-600 hover:text-yellow-700 hover:bg-yellow-50'
                                  : 'text-green-600 hover:text-green-700 hover:bg-green-50'
                              }`}
                              title={bill.status === 'paid' ? 'Marcar como em aberto' : 'Marcar como paga'}
                              data-testid={`btn-toggle-status-${bill.id}`}
                            >
                              {bill.status === 'paid' ? <XCircle size={18} /> : <CheckCircle size={18} />}
                            </Button>
                            {isAdmin && (
                              <>
                                <Button
                                ...
                                <Button
                                  size="sm"
                                  variant="ghost"
                                  onClick={() => setDeleteDialog({ open: true, billId: bill.id })}
      ...
      <AlertDialog open={deleteDialog.open} onOpenChange={(open) => setDeleteDialog({ open, billId: null })}>
