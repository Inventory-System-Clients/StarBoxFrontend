...
export const criarPeca = async (req, res) => {
  try {
    // Permitir apenas ADMIN ou GERENCIADOR
    if (req.usuario.role !== "ADMIN" && req.usuario.role !== "GERENCIADOR") {
      return res.status(403).json({ error: "Apenas ADMIN ou GERENCIADOR podem cadastrar peças." });
    }
