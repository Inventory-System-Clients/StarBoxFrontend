...
export const dashboardRelatorio = async (req, res) => {
  try {
    ...
    const faturamentoValorado = parseFloat(totaisDados.faturamentoTotal || 0);
    ...
    // Faturamento real = dinheiro + cartão/pix (usar valorFaturado como fallback)
    const faturamento = (dinheiro + pix) > 0 ? (dinheiro + pix) : faturamentoValorado;
      ...
      include: [
        {
          model: Produto,
          as: "produto",
          attributes: ["preco", "custoUnitario"],
        },
        {
          model: Movimentacao,
          attributes: [],
    ...
    const performanceMaquinas = await Promise.all(
      performanceRaw.map(async (p) => {
        ...
        const dinheiroMaq = parseFloat(p.dinheiro || 0);
        const pixMaq = parseFloat(p.pix || 0);
        const fatValorado = parseFloat(p.faturamento || 0);
        const fatReal = (dinheiroMaq + pixMaq) > 0 ? (dinheiroMaq + pixMaq) : fatValorado;

        return {
          nome: p.maquina.nome,
          faturamento: fatReal,
          dinheiro: dinheiroMaq,
          pix: pixMaq,
    ...
    res.json({
      ...
      graficoFinanceiro: timelineRaw.map((t) => {
        const dinDia = parseFloat(t.dinheiro || 0);
        const pixDia = parseFloat(t.pix || 0);
        const fatDia = parseFloat(t.faturamento || 0);
        const receitaDia = (dinDia + pixDia) > 0 ? (dinDia + pixDia) : fatDia;
        return {
          data: t.data,
          faturamento: receitaDia,
          dinheiro: dinDia,
          pix: pixDia,
          custo: 0,
        };
  ...
  try {
    ...
    const totais = movimentacoes.reduce(
      (acc, mov) => {
        ...
        acc.totalDinheiro += parseFloat(mov.quantidade_notas_entrada || 0);
        acc.totalCartaoPix += parseFloat(mov.valor_entrada_maquininha_pix || 0);
        return acc;
      },
      {
        totalFichas: 0,
    ...
    // Receita real = dinheiro + cartão/pix
    ...
    movimentacoes.forEach((mov) => {
      ...
      lojasMap[lojaNome].dinheiro = (lojasMap[lojaNome].dinheiro || 0) + parseFloat(mov.quantidade_notas_entrada || 0);
      lojasMap[lojaNome].cartaoPix = (lojasMap[lojaNome].cartaoPix || 0) + parseFloat(mov.valor_entrada_maquininha_pix || 0);
    ...
    const maquinasDetalhadas = Object.values(dadosPorMaquina).map((m) => ({
      ...
      produtosSairam: Object.values(m.produtosSairam)
        .map((p) => ({
          ...
          preco: p.produto.preco || 0,
          custoUnitario: p.produto.custoUnitario || 0,
          valorUnitario: p.produto.preco || p.produto.custoUnitario || 0,
        }))
